...
def split_data_by_day(full_symbol_df: pd.DataFrame, filter_length=None, limit_to_market_open=None):
    output_folder_path = config["resource_paths"]["polygon"]["daily_data_output_folder"]
...
def split_downloaded_data():
    csv_dir = config["resource_paths"]["polygon"]["polygon_bar_data_csvs"]
    csv_files = os.listdir(csv_dir)
    csv_files.sort()
    for filename in csv_files:
        csv_file_path = os.path.join(csv_dir, filename)
        if os.path.isfile(csv_file_path):
            print(f"Current symbol file: {filename}")
