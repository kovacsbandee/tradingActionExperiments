...
def download_aggs(symbol: str, from_: str, to: str, api_key) -> List[dict]:
    global global_request_counter
    if symbol is not None and isinstance(symbol, str):
        if '/' in symbol:
            symbol = symbol.replace("/", ".")
        url = f"https://api.polygon.io/v2/aggs/ticker/{symbol}/range/1/minute/{from_}/{to}?adjusted=true&sort=asc&limit=50000&apiKey={api_key}"
...
def get_bar_data(symbol: str, from_: str, to: str, api_key):
    aggregate_list = download_aggs(symbol, from_, to, api_key)
...
def create_csvs_by_symbol(symbol_list: list, from_: str, to: str, api_key):
    global global_request_counter
    for symbol in symbol_list:
        if global_request_counter < 5:
            symbol_df = get_bar_data(symbol, from_, to, api_key)
...
if __name__ == '__main__':
    create_csvs_by_symbol(nasdaq_symbols, "2023-12-01", "2023-12-31", api_key=POLYGON_API_KEY)
