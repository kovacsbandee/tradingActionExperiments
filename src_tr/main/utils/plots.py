...
def daily_time_series_charts(symbol_dict,
    ...
    for symbol in symbol_dict.keys():
        ...
        fig.update_yaxes(title = 'RSI', title_font=dict(color='black'), secondary_y=False, row=5, col=1)
        ...
        if epsilon > 0.0:
            ...
            fig.add_trace(go.Scatter(x=time_dimension,
                                 ...
                                 connectgaps=True), row=6, col=1)
            fig.update_yaxes(title = 'MACD', title_font=dict(color='blue'), secondary_y=False, row=6, col=1)
            
        #1 entry/close types
        fig.add_trace(go.Scatter(x=time_dimension,
                                 y=np.where(plot_df['current_capital_post_calculation'] < plot_df['current_capital_post_calculation'].mean() * 0.05,
                                            np.nan,
                                            plot_df['current_capital_post_calculation']),
                                 name='current_capital_post_calculation',
                                 mode='lines',
                                 marker=dict(color='blue'),
                                 connectgaps=True), row=7, col=1)
        fig.update_yaxes(title='Capital post calculation', title_font=dict(color='blue'), secondary_y=False, row=7, col=1)

        fig.add_trace(go.Histogram(x=plot_df['gain_per_qty_per_position_post_calculation'],
                                   name='gain_per_qty_per_position_post_calculation'), row=8, col=1)

        fig.add_trace(go.Histogram(x=plot_df['prev_position_monetary_yield_per_symbol'],
                                   name='prev_position_monetary_yield_per_symbol'), row=9, col=1)
        ...
        fig.add_trace(go.Scatter(x=plot_df.index,
                                 ...
                                 marker_color='red',
                                 marker_size=6), row=10, col=1)
        fig.update_yaxes(title = 'Entry/close types', title_font=dict(color='green'), secondary_y=False, row=10, col=1)
        fig.update_xaxes(showticklabels=True)
        fig.update_layout(title=f'{symbol} {date}.',
